...
function footer() {
  ...
  return {
    ...
    events,
    commits,
    submit(text: string | RunPrompt, mode?: RunPrompt["mode"]) {
      const next =
        typeof text === "string"
          ? mode
...
describe("run runtime queue", () => {
  ...
  test("emits an Ask agent label for the turn duration of an /ask prompt", async () => {
    const ui = footer()
    ...
    ui.submit({ text: "/ask", parts: [], command: { name: "ask", arguments: "" } })
    await task
  ...
  test("emits no agent label for the turn duration of a plain prompt", async () => {
    const ui = footer()
