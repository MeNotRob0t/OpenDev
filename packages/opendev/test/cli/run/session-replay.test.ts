...
describe("run session replay", () => {
  ...
  test("does not replay a turn summary for the auto-resume step that continues an /ask turn", () => {
    const askUser: SessionMessages[number] = {
      info: {
        id: "msg-user-ask",
        sessionID: "session-1",
        role: "user",
      ...
      parts: [
        {
          id: "msg-user-ask-text",
          sessionID: "session-1",
          messageID: "msg-user-ask",
          type: "text",
          text: "where is the main scripts",
        },
      ],
    }
    const askAssistant: SessionMessages[number] = {
      info: {
        ...assistantInfo("msg-ask", { parentID: "msg-user-ask", time: { created: 100, completed: 500 } }),
        mode: "ask",
        agent: "ask",
      },
      parts: [
    ...
    const out = replaySession({
      messages: [
        userMessage("msg-user-1", "Map the repo"),
        assistantMessage("msg-turn-1", "Mapping", {
          parentID: "msg-user-1",
          time: { created: 200, completed: 900 },
        }),
        askUser,
        askAssistant,
        assistantMessage("msg-resume", "Continuing", {
          parentID: "msg-user-1",
          time: { created: 1000, completed: 3000 },
        }),
      ],
    ...
    expect(out.commits.filter((commit) => commit.summary)).toEqual([
      expect.objectContaining({
        kind: "system",
        text: "▣ Build · gpt-5 · 700ms",
        messageID: "msg-turn-1",
      }),
      expect.objectContaining({
        kind: "system",
        text: "▣ Ask · gpt-5 · 400ms",
        messageID: "msg-ask",
      }),
    ])
  })
