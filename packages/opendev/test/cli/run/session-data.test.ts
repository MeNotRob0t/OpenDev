...
describe("run session data", () => {
  ...
  // Replays the exact `/ask` turn event sequence observed in the local DB:
  // user(ask) question, then an assistant(ask) answer streamed as deltas.
  // The answer must render like any other assistant turn.
  test("renders the /ask answer from the ask turn event sequence", () => {
    let data = createSessionData()

    data = reduce(data, user("msg-ask-user")).data
    data = reduce(
      data,
      text({ id: "prt-ask-user", messageID: "msg-ask-user", text: "where is the main scripts", time: { start: 1 } }),
    ).data
    data = reduce(
      data,
      assistant("msg-ask-assistant", { agent: "ask", mode: "ask", parentID: "msg-ask-user" }),
    ).data

    data = reduce(data, text({ id: "prt-ask-text", messageID: "msg-ask-assistant", text: "", time: { start: 2 } })).data

    let all = ""
    let out = reduce(data, delta("msg-ask-assistant", "prt-ask-text", "The primary scripts"))
    all += out.commits.map((commit) => commit.text).join("")
    out = reduce(out.data, delta("msg-ask-assistant", "prt-ask-text", " are located in packages/opencode"))
    all += out.commits.map((commit) => commit.text).join("")

    expect(all).toContain("The primary scripts are located in packages/opencode")

    out = reduce(
      out.data,
      text({
        id: "prt-ask-text",
        messageID: "msg-ask-assistant",
        text: "The primary scripts are located in packages/opencode",
        time: { end: 3 },
      }),
    )
    expect(out.commits).toEqual([])
  ...
  // A snapshot-style /ask answer (part.updated with the full text) also renders.
  test("renders a snapshot /ask answer", () => {
    let data = createSessionData()
    data = reduce(data, user("msg-ask-user")).data
    data = reduce(
      data,
