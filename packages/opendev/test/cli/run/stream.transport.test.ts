...
describe("run stream transport", () => {
  ...
  // Feeds the exact `/ask` turn event sequence captured from a live session
  // (user question, then an assistant(ask) answer streamed as deltas). The
  // answer must render in scrollback like any other assistant turn.
  test("renders the /ask answer as assistant scrollback", async () => {
    const src = eventFeed()
    const ui = footer()
    const transport = await createSessionTransport({
      sdk: sdk({
    ...
    try {
      ...
      src.push(askUser)
      src.push(textUpdated(textPart("prt-ask-user", "msg-ask-user", "where is the main scripts")))
      const askAssistant: SdkEvent = {
        id: "evt-msg-ask-assistant",
        type: "message.updated",
        properties: {
          sessionID: "session-1",
          info: {
            ...assistantMessage({ sessionID: "session-1", id: "msg-ask-assistant", parts: [] }).info,
            agent: "ask",
            mode: "ask",
            parentID: "msg-ask-user",
          } as SessionMessage["info"],
        },
      }
      src.push(askAssistant)
      src.push(textUpdated(textPart("prt-ask-text", "msg-ask-assistant", "")))
      src.push(textDelta("msg-ask-assistant", "prt-ask-text", "The primary scripts are"))
      await waitFor(() => ui.commits.find((commit) => commit.kind === "assistant" && commit.text === "The primary scripts are"))
      src.push(textDelta("msg-ask-assistant", "prt-ask-text", " located in packages/opencode"))
      src.push(
        textUpdated({
          ...textPart("prt-ask-text", "msg-ask-assistant", "The primary scripts are located in packages/opencode"),
          time: { start: 1, end: 2 },
        }),
      )
      await waitFor(() =>
        ui.commits.find((commit) => commit.kind === "assistant" && commit.text === " located in packages/opencode"),
      )

      const rendered = ui.commits
        .filter((commit) => commit.kind === "assistant")
        .map((commit) => commit.text)
        .join("")
      expect(rendered).toContain("The primary scripts are located in packages/opencode")
    } finally {
      src.close()
      await transport.close()
    }
