...
describe("MessageV2.filterCompacted", () => {
  ...
  it.instance("excludes /ask Q&A messages", () =>
    withSession(({ sessionID }) =>
      Effect.gen(function* () {
        const session = yield* SessionNs.Service
        const u1 = yield* addUser(sessionID, "hello")
        ...
        const askUserID = MessageID.ascending()
        yield* session.updateMessage({
          id: askUserID,
          sessionID,
          role: "user",
        ...
        const askAssistantID = MessageID.ascending()
        yield* session.updateMessage({
          id: askAssistantID,
          sessionID,
          role: "assistant",
        ...
        const result = MessageV2.filterCompacted(yield* MessageV2.stream(sessionID))
        expect(result).toHaveLength(2)
        expect(result.map((item) => item.info.id)).toEqual([u1, a1])
      }),
    ),
  )
})
