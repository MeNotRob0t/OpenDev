...
noLLMServer.instance(
  "built-in continue and also commands are listed",
  () =>
    Effect.gen(function* () {
      const commands = yield* Command.Service
      const names = (yield* commands.list()).map((c) => c.name)
      expect(names).toContain(Command.Default.CONTINUE)
      expect(names).toContain(Command.Default.ALSO)
      expect(names).toContain(Command.Default.SEED)
      expect(names).toContain(Command.Default.ASK)
    }),
  { config: cfg },
)

noLLMServer.instance(
  "/also throws when there is nothing to append",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      if (Exit.isFailure(exit)) {
        const err = Cause.squash(exit.cause)
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) expect(err.data.message).toBe("Nothing to append.")
      }
    }),
  { config: cfg },
)
...
it.instance(
  "/also merges arguments into the last user message without creating a new user message",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      expect(before.filter((m) => m.info.role === "user").length).toBe(1)
      ...
      const users = after.filter((m) => m.info.role === "user")
      expect(users.length).toBe(1)
      const texts = users[0]?.parts.filter((p) => p.type === "text")
      expect(texts?.at(-1)?.text).toBe("original\n\nextra")
    }),
)

it.instance(
  "/also appends a new assistant reply after the previous one",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      expect(users.length).toBe(1)
      const userTexts = users[0]?.parts.filter((p) => p.type === "text")
      expect(userTexts?.at(-1)?.text).toBe("hello\n\nextra")
      expect(assistants.length).toBe(2)
      expect(assistants[0]?.parts.some((p) => p.type === "text" && p.text === "hi there")).toBe(true)
      expect(assistants[1]?.parts.some((p) => p.type === "text" && p.text === "second reply")).toBe(true)
    }),
)

noLLMServer.instance(
  "/ask throws when there is nothing to ask",
  () =>
    Effect.gen(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      if (Exit.isFailure(exit)) {
        const err = Cause.squash(exit.cause)
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) expect(err.data.message).toBe("Nothing to ask.")
      }
    }),
  { config: cfg },
)
...
it.instance(
  "/ask answers the question read-only and resumes the previous task",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const chat = yield* sessions.create({ title: "Pinned" })
      yield* seed(chat.id, { finish: "stop" })
      yield* llm.text("the ask reply")
      yield* llm.text("resumed reply")

      const result = yield* prompt.command({
        sessionID: chat.id,
        command: Command.Default.ASK,
        arguments: "what is the current task?",
      })
      ...
      const msgs = yield* sessions.messages({ sessionID: chat.id })
      const asks = msgs.filter((m) => m.info.agent === "ask")
      expect(asks.length).toBe(2)
      expect(asks[0]?.info.role).toBe("user")
      expect(asks[0]?.parts.some((p) => p.type === "text" && p.text === "what is the current task?")).toBe(true)
      expect(asks[1]?.info.role).toBe("assistant")
      expect(asks[1]?.parts.some((p) => p.type === "text" && p.text === "the ask reply")).toBe(true)
      expect(msgs.filter((m) => m.info.role === "assistant").length).toBe(3)

      // The ask request has no tools, the resume request has the normal tools.
      const inputs = yield* llm.inputs
      const askMessages = inputs[0]?.messages
      const resumeMessages = inputs[1]?.messages
      expect(JSON.stringify(askMessages)).toContain("what is the current task?")
      expect(Object.keys((inputs[0]?.tools ?? {}) as object)).toHaveLength(0)
      // The Q&A is excluded from the model context of the resumed task.
      expect(JSON.stringify(resumeMessages)).not.toContain("what is the current task?")
      expect(JSON.stringify(resumeMessages)).toContain("hello")
    }),
)

it.instance(
  "/ask on a fresh session only answers and does not loop",
  () =>
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const prompt = yield* SessionPrompt.Service
      ...
      const result = yield* prompt.command({
        sessionID: chat.id,
        command: Command.Default.ASK,
        arguments: "where am I?",
      })

      expect(result.info.role).toBe("assistant")
      expect(result.info.agent).toBe("ask")
      expect(yield* llm.calls).toBe(1)
    }),
)
