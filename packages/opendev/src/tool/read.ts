...
>(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      // Dedup: check if file was already read this session
      const alreadyRead = ctx.messages.some((msg) =>
        msg.parts.some(
          (p) => p.type === "tool" && p.tool === "read" && p.state.input && typeof p.state.input === "object" && "filePath" in p.state.input && path.resolve(instance.directory, String(p.state.input.filePath)) === filepath,
        ),
      )
      if (alreadyRead) {
        return {
          title,
          output: `<memory-hint>\nFile ${filepath} was already read. Use m(c:"E", p:"/Important_Files${filepath}") for cached content.\n</memory-hint>`,
          metadata: { preview: "already read", truncated: false, loaded: [] },
        }
      }
