...
function defaultModelFromConfig(
  ...
  // a default. Configured model, opendev provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const opendevProvider = providers[ProviderV2.ID.make("opendev")]
  const opendevModel = opendevProvider ? Provider.sort(Object.values(opendevProvider.models))[0] : undefined
  if (opendevProvider && opendevModel) return { providerID: opendevProvider.id, modelID: opendevModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenDev service failure", service })
}
