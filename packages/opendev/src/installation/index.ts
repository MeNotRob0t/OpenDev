...
const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "anomalyco/tap/opendev"])
      if (tapFormula.includes("opendev")) return "anomalyco/tap/opendev"
      const coreFormula = yield* text(["brew", "list", "--formula", "opendev"])
      if (coreFormula.includes("opendev")) return "opendev"
      return "opendev"
    })
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".opendev", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "opendev"]) },
          { name: "scoop", command: () => text(["scoop", "list", "opendev"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "opendev"]) },
        ]
