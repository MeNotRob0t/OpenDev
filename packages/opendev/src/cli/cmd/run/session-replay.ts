...
function summaryMessageIDs(messages: SessionMessages): ReadonlySet<string> {
  ...
  const precedingUser = new Map<number, string>()
  ...
  for (let idx = 0; idx < messages.length; idx += 1) {
    ...
    if (message.info.role === "user") {
      lastUser = message.info.id
      userIDs.add(lastUser)
    }

    if (lastUser) {
      precedingUser.set(idx, lastUser)
    }
  }
  ...
  for (let idx = messages.length - 1; idx >= 0; idx -= 1) {
    ...
    // Skip out-of-order continuation steps (e.g. the auto-resume turn after
    // /ask) whose parent is a user message that is not the preceding user turn.
    const parentID = message.info.parentID
    if (parentID && userIDs.has(parentID) && parentID !== precedingUser.get(idx)) {
      continue
    }
