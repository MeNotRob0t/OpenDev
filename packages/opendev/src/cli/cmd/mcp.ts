...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in opendev.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .opendev/ subdirectory too)
  const candidates = [path.join(baseDir, "opendev.json"), path.join(baseDir, "opendev.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".opendev", "opendev.json"), path.join(baseDir, ".opendev", "opendev.jsonc"))
  }
  ...
  // Default to opendev.json if none exist
  return candidates[0]
}
