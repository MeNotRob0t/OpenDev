  ...
  Effect.gen(function* () {
    ...
    const systemPaths = Effect.fn("Instruction.systemPaths")(function* () {
      ...
      if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
        ...
        // Load EDITS.md independently — it's a formatting dictionary, not an instruction file.
        const edits = yield* fs
          .findUp("EDITS.md", ctx.directory, ctx.worktree)
          .pipe(Effect.catch(() => Effect.succeed([] as string[])))
        edits.forEach((item) => paths.add(path.resolve(item)))
        const globalEdits = path.join(global.config, "EDITS.md")
        if (yield* fs.existsSafe(globalEdits)) {
          paths.add(path.resolve(globalEdits))
        }
      }
