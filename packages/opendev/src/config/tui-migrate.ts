...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from opendev.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "opendev"),
    ...(await Filesystem.findUp(["opendev.json", "opendev.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opendev"))
  }
