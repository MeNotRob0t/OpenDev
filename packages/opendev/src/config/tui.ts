...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old opendev.json shape) still get their settings applied.
...
// Every config dir we may read from: global config dir, any `.opendev`
// folders between cwd and home, and OPENCODE_CONFIG_DIR.
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".opendev") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".opendev") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
