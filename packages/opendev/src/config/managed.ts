...
const MANAGED_PLIST_DOMAIN = "ai.opendev.managed"

// Keys injected by macOS/MDM into the managed plist that are not OpenDev config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/opendev"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "opendev")
    default:
      return "/etc/opendev"
  }
}
