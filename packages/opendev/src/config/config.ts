...
function globalConfigFile() {
  const candidates = ["opendev.jsonc", "opendev.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opendev.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "opendev.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".opendev") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["opendev.json", "opendev.jsonc"]) {
          ...
          // Auto-discovered plugins under `.opendev/plugin(s)` are already local files, so ConfigPlugin.load
